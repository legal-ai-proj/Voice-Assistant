...
async def get_branch_with_chain(db: AsyncSession, branch_id: UUID) -> tuple[Branch, Chain] | None:
...
async def get_branch_hours(db: AsyncSession, branch_id: UUID) -> list[BranchHours]:
...
async def get_active_services(db: AsyncSession, branch_id: UUID) -> list[Service]:
    result = await db.execute(
...
async def get_active_staff(db: AsyncSession, branch_id: UUID) -> list[Staff]:
    result = await db.execute(
...
async def get_active_products(db: AsyncSession, branch_id: UUID) -> list[Product]:
    result = await db.execute(
...
async def get_branch_settings(db: AsyncSession, branch_id: UUID) -> BranchSettings | None:
